...
const Home = () => {
  ...
  const createClass = async () => {
    ...
    await axios.post(process.env.REACT_APP_AUTH_URL + '/create_class' || 'http://localhost:4001/create_class', {
      userId, title
    });
  }
  ...
  const joinClass = async () => {
    ...
    await axios.post(process.env.REACT_APP_AUTH_URL + '/add_class' || 'http://localhost:4001/add_class', {
      userId, classId
    });
  };

  const fetchClasses = async () => {
    const res = await axios.get(process.env.REACT_APP_AUTH_URL + '/get_classes' ||'http://localhost:4001/get_classes/' + userId);
